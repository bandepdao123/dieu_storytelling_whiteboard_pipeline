-- Historical schema: v13_inherited; generated from actual Database constructor, not relabelled current schema.
BEGIN TRANSACTION;
...
CREATE TABLE publication_intents(token TEXT PRIMARY KEY,project_id TEXT NOT NULL REFERENCES projects(id),source_path TEXT NOT NULL,identity_json TEXT NOT NULL,journal_json TEXT NOT NULL,state TEXT NOT NULL CHECK(state IN ('INTENT','PREPARED','ABORTED','MANUAL_REVIEW')),created_at TEXT NOT NULL,updated_at TEXT NOT NULL,error TEXT);
...
CREATE INDEX idx_attempts_scene_number ON attempts(scene_id,epoch,number);
...
CREATE INDEX idx_publication_intent_state ON publication_intents(state,created_at);
COMMIT;
